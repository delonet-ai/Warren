...
**[P1] E2E не ловит нерабочий VLESS-туннель**

Проверка «foreign HTTPS через VLESS» ходит на `google.com`, которого нет в списках Podkop: трафик идёт напрямую, и тест зелёный даже когда туннель не работает. 2026-09-25 на стенде все соединения через `outbound/vless[main-out]` падали с `reality verification failed` при `40 PASS / 0 FAIL`. Нужно проверять домен из списков (например, `instagram.com`: fake-IP `198.18.0.0/15` + успешный HTTPS) и отсутствие ошибок sing-box в `logread`.

**[P1] Стенд: `reality verification failed` роутер → VPS**

Роутер: sing-box `1.13.21` (Podkop `0.7.22`), VPS: Xray `26.7.11` (3x-ui). Ключи, shortId, SNI (`login.vk.com`) и время совпадают; ClientHello доходит до VPS; Xray-клиент с теми же параметрами через loopback на VPS работает (HTTP 200). Осталось различить: несовместимость sing-box 1.13 ↔ Xray 26.7 или вмешательство на пути (проверка sing-box-клиентом на VPS через loopback, затем смена SNI через SNI apply flow).

**[P2] Пин установщика Podkop не действует**

На OpenWrt `25.12.5` Podkop ставится из официального фида (`feeds/utilities/podkop`, установлен `0.7.22`), а не нашим pinned installer `0.7.21`. Решить: пинить версию пакета из фида (`apk add podkop=<ver>`) или явно принять фид как источник и проверять только поведение.
...
### Milestone 19 — AmneziaWG Obfuscation by Default

Статус: `planned` (решение 2026-09-25: остаёмся на AWG для доступа «устройства → роутер», VLESS-вход на роутере не делаем).

Проблема: текущие дефолты `Jc=4 Jmin=40 Jmax=70 S1=0 S2=0 H1..H4=1..4`, порт `51820` дают рукопожатие, побайтово совпадающее с обычным WireGuard (тот же тип сообщения и размер 148 байт, стандартный порт). Обфусцированы только junk-пакеты перед рукопожатием, поэтому сигнатурный DPI распознаёт трафик как WireGuard.

Целевые дефолты для новой установки (генерируются один раз на сервер и хранятся в UCI `network.awg0`):

- `H1..H4` — случайные, попарно различные, вне `1..4`, в диапазоне `5..2147483647`;
- `S1`, `S2` — случайные ненулевые `15..150`, при этом `S1 + 56 != S2` (иначе размеры init/response снова совпадают);
- `Jc` `4..8`, `Jmin`/`Jmax` в пределах `40..1000`, `Jmin < Jmax`;
- порт — случайный из `20000..60000` вместо `51820` (переопределяется явно);
- AWG 2.0: `I1` (имитация другого протокола) — только опцией, после проверки поддержки в клиентах AmneziaVPN/AmneziaWG для macOS/iOS/Android.

Единый источник параметров:

- после настройки сервера все генераторы клиентских конфигов читают параметры из UCI `network.awg0.awg_*`, а не из дефолтов в коде: `lib/amneziawg.sh` (`create_amneziawg_peer_config`) и `payload/warren-tg-bot` (`amz_obfuscation_defaults` сейчас захардкожен `H=1..4, S=0`);
- LuCI, shell и TG выдают одинаковые конфиги.

Существующие установки:

- параметры не меняются автоматически — иначе все выданные клиенты перестанут подключаться;
- отдельное действие «Перегенерировать обфускацию AWG» (shell, LuCI, TG): новые параметры → перезапуск `awg0` → перевыпуск конфигов/QR всем клиентам с явным предупреждением;
- diagnostics помечает `WARN`, если на сервере стоят WireGuard-совместимые значения (`S1=S2=0`, `H=1..4`) или порт `51820`.

Acceptance:

- fresh install генерирует параметры по правилам выше; повторный прогон не меняет их;
- конфиг клиента из shell, LuCI и TG совпадает с UCI сервера;
- клиент AmneziaWG с новым конфигом подключается (hardware E2E: handshake + трафик);
- rotate перевыпускает конфиги и старые перестают работать;
- tests: генератор соблюдает ограничения (`S1+56≠S2`, различные `H`, диапазоны), парсинг параметров из UCI.

Проверка эффекта (вне CI): рукопожатие WG vs AWG-по-новым-дефолтам из реально используемых сетей (мобильные операторы, чужой Wi-Fi).
