...
watchdog_write_worker() {
  ...
  cat > "$WARREN_WATCHDOG_BIN" <<'WATCHDOG_EOF'
#!/bin/sh
set -u
...
engine_running() {
  for comm_file in /proc/[0-9]*/comm; do
    ...
    [ "$process_name" = "sing-box" ] && return 0
  done
  ps w 2>/dev/null | grep -Eq '[/]usr/bin/sing-box[[:space:]]'
}
...
run_once() {
  ...
  reason="$(health_reason)" && healthy=1 || healthy=0
  if [ "$healthy" = "1" ]; then
...
watchdog_write_init() {
  cat > "$WARREN_WATCHDOG_INIT" <<'WATCHDOG_INIT_EOF'
...
start_service() {
  procd_open_instance
  procd_set_param command /usr/libexec/warren/warren-watchdog daemon
